...
bank_withdrawals_col   = db["bank_withdrawals"]    # manual withdrawals from bank accounts
...
def _sum_withdrawals_out(bank_oid: ObjectId) -> float:
    """Sum of manual withdrawals from this bank (bank_withdrawals.bank_id)."""
    try:
        pipe = [
            {"$match": {"bank_id": bank_oid}},
            {"$group": {"_id": None, "total": {"$sum": "$amount"}}},
        ]
        row = next(bank_withdrawals_col.aggregate(pipe), None)
        return _safe_float(row["total"]) if row else 0.0
    except Exception:
        return 0.0
...
def bank_profile(bank_id: str):
    ...
    withdrawals = list(
        bank_withdrawals_col.find({"bank_id": bank_oid}).sort("txn_date", -1)
    )
...
def withdraw(bank_id: str):
    ...
    bank_withdrawals_col.insert_one(doc)
