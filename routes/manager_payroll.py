...
def payroll_month_payload(month):
    ...
    for a in agents:
        ...
        rows.append({
            ...
            "deductions": existing.get("deductions") or [],
        })
    ...
    totals = None
    if payroll:
        totals = payroll.get("totals") or _compute_totals(payroll.get("items", []), global_deds)
    else:
...
def payroll_save():
    ...
    for it in items:
        ...
        clean_items.append({
            ...
            "deductions": (prev.get("deductions") if prev else []),
        })
...
def payroll_details(pid):
    ...
    global_deds = payroll.get("global_deductions") or []
    items = payroll.get("items") or []
    totals = payroll.get("totals") or _compute_totals(items, global_deds)
