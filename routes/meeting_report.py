...
def agent_metrics():
    ...
    if scope not in ("agent", "manager", "branch"):
        scope = "agent"
    ...
    # ---------- Resolve SUBJECT (agent / manager / branch) & agent_ids ----------
    ...
    if scope == "agent":
        ...
        mgr_oid = manager["_id"]
        mgr_id_str = str(mgr_oid)

        # All agents under this manager (supports manager_id stored as ObjId or string)
        agent_query = {
            "role": "agent",
            "$or": [
                {"manager_id": mgr_oid},
        ...
        if not branch:
            return jsonify(ok=False, message="branch is required for scope=branch"), 400

        agents = list(
            users_col.find(
                {"role": "agent", "branch": branch},
                {"_id": 1}
            )
        )
        agent_ids = [str(a["_id"]) for a in agents]
        ...
        subject = {
            "type": "branch",
            "id": branch,
            "name": f"{branch} Branch" if branch else "All branches",
            "branch": branch,
            "phone": "",
            "image_url": "",
            "manager_name": "",
    ...
    # If manager/branch has no agents, return an empty but OK payload
    if scope in ("manager", "branch") and not agent_ids:
        empty_data = {
            "ok": True,
            "agent": subject,
    ...
    # Customers under this agent/manager/branch (ALL TIME)
    if scope == "agent":
