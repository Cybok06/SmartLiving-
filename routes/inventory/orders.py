...
def _order_items_summary(order_doc: Dict[str, Any]) -> Tuple[int, int]:
    ...
    return len(items), remaining_total
...
def _deliver_line_core(*, user: Dict[str, Any], order_id: str, line_id: str, qty: int) -> str:
    ...
    # inventory transfer: warehouse -> manager inventory
    ...
    orders_col.update_one(
        {"_id": order["_id"]},
        {"$set": {"items": items, "status": sta, "updated_at": now}},
    )
...
def deliver_line():
    ...
    sta = _deliver_line_core(user=user, order_id=str(order_id), line_id=str(line_id), qty=qty)
...
def deliver_all_remaining():
    ...
    for it in (order.get("items") or []):
        ...
        if remaining <= 0:
            continue
        last_status = _deliver_line_core(user=user, order_id=order_id, line_id=str(line_id), qty=remaining)
        did += 1
