...
def _admin_required() -> bool:
    """Return True if admin logged in; on AJAX caller can act on 401."""
...
def customer_activity_manual():
    ...
    # Optional: resolve admin name if stored in users_col
    created_by_name = ""
    try:
        if created_by:
            admin_oid = ObjectId(created_by)
            admin_doc = users_col.find_one({"_id": admin_oid})
            if admin_doc:
                created_by_name = admin_doc.get("name", "") or ""
    except Exception:
        created_by_name = ""
