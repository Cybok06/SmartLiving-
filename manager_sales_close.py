...
def _agent_total_unclosed_all_dates(agent_id_str: str) -> float:
    """Sum total_amount across ALL dates for one agent."""
    pipeline = [
...
def manager_withdraw():
    ...
    Behavior:
      - Withdraws without a date picker:
          1) Try to debit TODAY's sales_close doc.
          2) If not enough, try the MOST RECENT doc with sufficient balance.
      - Credits the MANAGER'S TODAY doc.
      - Returns refreshed:
          - agent’s TOTAL unclosed (all dates),
    ...
    # --- 1) Try to debit TODAY's doc ---
    chosen_date = today
    agent_filter = {"agent_id": str(agent_id), "date": chosen_date, "total_amount": {"$gte": amount}}
    agent_update = {
    ...
    res = sales_close_col.update_one(agent_filter, agent_update)

    # --- 2) If today failed, try most recent doc with enough balance ---
    if res.modified_count == 0:
        cursor = sales_close_col.find(
            {"agent_id": str(agent_id), "total_amount": {"$gte": amount}},
            {"date": 1}
        ).sort([("date", -1), ("updated_at", -1)]).limit(1)
        doc = next(cursor, None)
        if not doc:
            total_all = _agent_total_unclosed_all_dates(str(agent_id))
        ...
        res2 = sales_close_col.update_one(agent_filter, agent_update)
        if res2.modified_count == 0:
            total_all = _agent_total_unclosed_all_dates(str(agent_id))
            msg = f"Insufficient balance. Agent total across all days: GHS {total_all:,.2f}"
            return (jsonify(ok=False, message=msg, available=f"{total_all:,.2f}"), 409) if _is_ajax(request) else (msg, 409)

    # --- 3) Credit MANAGER'S TODAY doc ---
    ...
    payload = {
        "ok": True,
        "message": f"Withdrew GHS {amount:,.2f} (debited {chosen_date}) and credited manager account.",
    ...
    return jsonify(payload) if _is_ajax(request) else (
        f"OK. Agent total: {payload['available']}, "
