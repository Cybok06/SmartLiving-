...
def _build_base_match(
    ...
    """Base filter – only this manager’s customers, plus optional agent/search/tag filters."""
    base_match: Dict[str, Any] = {"manager_id": manager_oid}
...
def customers_data():
    ...
    base_match = _build_base_match(manager_oid, search_term, agent_id_param, tag_filter)
    ...
    if agent_oids:
        for a in users_col.find({"_id": {"$in": list(set(agent_oids))}}, {"_id": 1, "name": 1}):
