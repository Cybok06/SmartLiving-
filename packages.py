...
def list_packages():
    ...
    if q:
        ...
        rows = [r for r in rows if
                ...
                ((r.get("product") or {}).get("name") and q_low in r["product"]["name"].lower())
        ]
    ...
    if request.args.get("export") == "1":
        def _gen():
            ...
            for p in rows:
                amt = (p.get("product") or {}).get("total", "")
                ds  = p.get("submitted_at").strftime("%Y-%m-%d") if isinstance(p.get("submitted_at"), datetime) else ""
                yield f"{p.get('agent_name','')},{p.get('agent_branch','')},{p.get('customer_name','')},{(p.get('product') or {}).get('name','')},{amt},{ds},{p.get('status','')}\n"
