...
def _build_user_filter(branch: str = "", role: str = "", status: str = "") -> Dict[str, Any]:
    ...
    # Only count actual staff roles (tune as you want)
    q["role"] = {"$in": ["agent", "manager", "admin", "executive", "inventory"]}
...
def dashboard():
    ...
    # Branch list for selector (dynamic from DB)
    branch_list = users_col.distinct("branch", {"branch": {"$ne": ""}})
...
def dashboard_data():
    ...
    # ---------------- Users KPIs ----------------
    u_q = _build_user_filter(branch=branch, role=role, status=status)

    total_employees = users_col.count_documents(u_q)

    active_q = dict(u_q)
    active_q["$or"] = [{"employment_status": "Active"}, {"status": "Active"}]
    active_employees = users_col.count_documents(active_q)

    probation_q = dict(u_q)
    probation_q["employment_status"] = {"$in": ["Probation", "On Probation"]}
    on_probation = users_col.count_documents(probation_q)

    recruit_q = dict(u_q)
    recruit_q["recruitment_stage"] = {"$exists": True, "$ne": "Full Staff (After 3 Months)"}
    recruitment_in_progress = users_col.count_documents(recruit_q)
    ...
    # if filtering by branch, chart becomes single bar (still ok)
    match_q = _build_user_filter(branch=branch, role=role, status=status)
    ...
    # recruitment stages distribution
    for row in users_col.aggregate(
        [
            {"$match": {**match_q, "recruitment_stage": {"$exists": True}}},
    ...
    candidates = list(
        users_col.find(
            b_q,
            {"name": 1, "branch": 1, "role": 1, "dob": 1, "employee_code": 1, "image_url": 1},
        ).limit(800)
    )
