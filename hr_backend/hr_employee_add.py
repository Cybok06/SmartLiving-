...
def add_employee():
    ...
    # ---------------- Validation ----------------
    if not name or not username or not phone or not role or not branch:
        msg = "Name, username, phone, role and branch are required."
        if request.headers.get("X-Requested-With") == "XMLHttpRequest":
    ...
    # If role is agent, manager_id required and must be manager
    manager_oid = None
    if role == "agent":
